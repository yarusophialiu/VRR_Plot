...
if __name__ == "__main__":
    ...
    # # the number of data points in that 1 bitrate
    ...
    exp_type = "res_only" # "baseline"  "res_only"
    folder_path = f"csv/{exp_name}/processed_{exp_type}"

    type_title = {'baseline': 'Baseline', 'res_only': "Resolution only"}
    ...
    if PROCESS_CSV:
        ...
        csv_val_dict = process_csv_value(folder_path, DEBUG=DEBUG)
    ...
    if data_type == "static":
        scenes_to_keep = ["breakfastroom", "salledebain", "sibenik"]
        N = 10 * 9 # 90 points in 1 bitrate, number oberservers * number of clips
    elif data_type == "dynamic":
        scenes_to_keep = ["makeway", "rogue", "school"]
        N = 10 * 6 # 60 points in 1 bitrate, number oberservers * number of clips
    elif data_type == "mixed":
        scenes_to_keep = ["breakfastroom", "salledebain", "sibenik", "makeway", "rogue", "school"]
        N = 10 * 9 + 10 * 6
    # if "sibenik" in scenes_to_keep:
    #     N = 10 * 9 # 90 points in 1 bitrate, number oberservers * number of clips
    # else:
    #     N = 10 * 6 # 60 points in 1 bitrate, number oberservers * number of clips
    
    if PLOT:
        csv_val_dict = process_csv_value(folder_path, scenes_to_keep=scenes_to_keep, DEBUG=DEBUG)
        ...
        for i, (bitrate, speed) in enumerate([(b, s) for b in bitrates for s in speeds]):
            ...
            # significance value
            print(f'pvalue speed{speed}, bitrate {bitrate} {round(p_value[speed][bitrates.index(bitrate)].pvalue, 2)}')
        ...
        # if "sibenik" in scenes_to_keep:
        #     plt.ylabel(f"Static scenes - probability of selecting ours", fontsize=15)
        #     # plt.ylabel(f"Static scenes - {type_title[exp_type]} - probability of selecting ours", fontsize=15)
        # else:
        #     plt.ylabel(f"Dynamic scenes - probability of selecting ours", fontsize=15)
        plt.tight_layout()
